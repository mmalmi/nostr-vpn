...
impl SpilmanClientAsyncNetworking for Mint {
    async fn call_mint_swap(&self, _: &str, request: &str) -> Result<String, String> {
        ...
        let signatures: Vec<_> = request.outputs.iter().map(|message| {
            ...
            signature.add_dleq_proof(&message.blinded_secret, &self.secret).unwrap();
            signature
        }).collect();
...
async fn funding_preserves_requested_capacity_and_recovers_the_same_channel() {
    let mint = Mint { secret: SecretKey::generate(), swaps: AtomicUsize::new(0) };
    ...
    let token = Token::new("https://mint.example".parse().unwrap(),
        [1, 2, 8].map(|amount| Proof::new(Amount::from(amount), keyset.keyset_id,
            Secret::new(format!("input-{amount}")), mint.secret.public_key())).to_vec(),
    ...
    let opened = open_streaming_route_cashu_spilman_channel_from_token_with_networking(
        directory.path(), request.clone(), &mint).await.unwrap();
    ...
    let recovered = open_streaming_route_cashu_spilman_channel_from_token_with_networking(
        directory.path(), retry, &mint).await.unwrap();
    ...
    assert!(open_streaming_route_cashu_spilman_channel_from_token_with_networking(
        other_directory.path(), underfunded, &mint).await.is_err());
    assert_eq!(mint.swaps.load(Ordering::SeqCst), 1, "reject before spending the token");
}
